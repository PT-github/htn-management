    ...
    <div class="table certificate-management">
        <div class="container">
            <div class="handle-box">
                <el-input v-model="name" placeholder="广告名称" class="handle-input mr10"></el-input>
            ...
            <el-table tooltip-effect="light" v-loading="loading" :data="tableData" border style="width: 100%"
                ...
                <el-table-column align="center" prop="name" fixed label="名称" width="100"></el-table-column>
                ...
                <el-table-column align="center" label="是否显示" width="100">
                    <template slot-scope="scope">
                        <span>{{ displayConfig[scope.row.isDisplay] }}</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" label="开始时间" width="150">
                    <template slot-scope="scope">
                        <span>{{ scope.row.effTime | moment }}</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" label="结束时间" width="150">
                    <template slot-scope="scope">
                        <span>{{ scope.row.expTime | moment }}</span>
                    </template>
                </el-table-column>
                <el-table-column :show-overflow-tooltip="true" prop="remark" label="描述"></el-table-column>
                <el-table-column fixed="right" label="操作" width="220">
                    <template slot-scope="scope">
                        <el-button size="small" @click="_editCerti(scope.row)">编辑</el-button>
                        <template v-if="scope.row.isDisplay === '0'">
                            <el-button size="small" type="primary" @click="displayFun(scope.row, '1')">显示</el-button>
                        </template>
                        <template v-else>
                            <el-button size="small" type="primary" @click="displayFun(scope.row, '0')">隐藏</el-button>
                        </template>
...
<script>
    import {queryAdvert, modifyAdvert, deleteAdvert, setAdvertIsDisplay} from "@/api/service";
    export default {
        ...
        methods: {
            displayFun (data, o) {
                ...
                setAdvertIsDisplay({id: data.id, isDisplay: o})
                    .then(() => {
                        loading.close();
                        data.isDisplay = o
                    })
